...
Continuous health monitoring has become increasingly important as mobile health systems and wearable biosensors move beyond episodic clinical measurements. Hydration assessment is a meaningful use case because dehydration affects physical performance, kidney function, metabolic regulation, and general wellness, yet it is often assessed through subjective reporting or delayed laboratory tests. AquaTrack focuses on a wearable multimodal approach that can use physiological, biochemical, and environmental signals together rather than relying on a single indicator.

Recent research has explored dehydration screening through electrodermal activity, impedance, body temperature, smartphone photoplethysmography, and other non-invasive signals. These approaches can classify autonomic and contextual responses, but they may miss direct chemical evidence of fluid loss. Human sweat provides a useful non-invasive alternative because it carries electrolytes and metabolites such as chloride, osmolality-related indicators, lactate, glucose, and urea. In AquaTrack, sweat and salivary indicators are treated as primary biomarker signals, while environmental and activity features provide context for interpreting those biomarkers.

A wearable sensing layer captures sweat, saliva, skin, and environmental signals. These signals pass through data acquisition, preprocessing, feature engineering, context detection, and dual-pipeline machine learning before the final dehydration risk label is served to the mobile application.
...
Jain et al. propose a spiral dermal patch with discrete colorimetric indicators for sweat-rate quantification. The time taken for sweat to travel through the patch geometry indicates sweat production, showing how microfluidic channel design can support quantitative sweat analysis.
...
The project methodology begins with hydrogel synthesis using sodium alginate and polyvinyl alcohol trials. The successful sodium alginate patches are evaluated for flexibility, swelling capacity, and colorimetric response to urea. In parallel, two datasets are prepared: a biomarker dataset containing sweat, salivary, body-composition, impedance, temperature, and activity features, and a context dataset containing skin, environmental, and time-related features.

The computational workflow applies median imputation, winsorization, logarithmic transformation, robust scaling, composite feature engineering, feature selection, classifier training, and weighted prediction fusion. The final architecture exposes the trained model through a lightweight FastAPI backend and visualizes predictions using a React Native mobile dashboard.
...
\begin{itemize}
\item Chapter 2 discusses the fundamentals of sweat sensing, hydrogels, dehydration biomarkers, and chronic kidney disease relevance.
