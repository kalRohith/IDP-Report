...
Once a model is trained, we need to know how well it performs. For beginners, the most common metrics used to "score" a model are:
\begin{itemize}
\item \textbf{Accuracy}: The percentage of total guesses the model got right. While simple, it can be misleading if one class is much more common than others.
\item \textbf{Precision}: Out of all the times the model predicted "High Risk," how many were actually correct? It measures the quality of the positive predictions.
\item \textbf{Recall}: Out of all the actual "High Risk" cases, how many did the model manage to find? it measures the ability to find all relevant cases.
\item \textbf{F1-Score}: A balanced score that combines both Precision and Recall into a single number. This is often the best way to see the overall health of a model.
\end{itemize}
